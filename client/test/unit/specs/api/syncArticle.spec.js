...
import api from '@/api/syncArticle';

describe('Unit | API | syncArticle api', () => {
  describe('#syncArticle', () => {
    ...
    it('should fetch API with the good params', () => {
      // given
      const expectedUrl = `${process.env.API_URL}api/admin/articles/${id}/sync`;
      const expectedOptions = { headers: { 'Content-Type': 'application/json' } };

      // when
      const promise = api.syncArticle(id);
    ...
    it('should return a rejected promise when an error is thrown', (done) => {
      ...
      // when
      const promise = api.syncArticle(accessToken);
