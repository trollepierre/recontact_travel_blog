...
// Load .env.local then .env before the config below reads process.env
...
// '' means "same origin" for the build served by the back; the dev proxy needs a real target
...
// Prerendered routes generated by scripts/prerender-routes.mjs
...
export default defineNuxtConfig({
	...
	app: {
		head: {
			...
			script: [
				// Injected at runtime by the back (back/src/infrastructure/env/static-env.js); elsewhere
				// this is the inert placeholder from front/public/env.js. A classic script, so it runs
				// before the Nuxt entry, which is a module and therefore implicitly deferred — required
				// because api-service.js computes its baseURL at import time.
